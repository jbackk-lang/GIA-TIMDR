# core/phase_winding_oam_ms_bridge.py
"""
phase_winding_oam_ms_bridge.py -- implementacja PRE-REJESTROWANEJ (patrz
docs/geometry/PREREG_PHASE_WINDING_OAM_MS_BRIDGE.md) syntetycznej
kontroli pozytywnej/negatywnej dla mostu "sygnal 1D (M/S) -> transformata
Hilberta -> chwilowa faza -> winding fazy (inspirowane topologicznym
ladunkiem OAM fotonu, e^{i*l*phi}) -> pipeline anty-numerologiczny".

Piaty kandydat, jakosciowo inny od poprzednich czterech (torsja,
winding/crossing po trajektorii embeddingu, homologia perzystentna --
wszystkie mierzyly lokalna/globalna nieregularnosc KSZTALTU trajektorii
i przegraly z entropia szumu). Ten mierzy SYSTEMATYCZNA AKUMULACJE
FAZY w czasie -- inny mechanizm odpornosci na szum (dryf liniowy vs
blad losowy dyfuzyjny).

Zero embeddingu opozniajacego -- dziala bezposrednio na sygnale 1D.
...
# ---------------------------------------------------------------------
# 2. Metryka: winding fazy przez transformate Hilberta (zamrozona)
# ---------------------------------------------------------------------


def phase_winding_fn(signal_1d: np.ndarray) -> float:
    x = np.asarray(signal_1d, dtype=float)
    if len(x) < 4 or x.std() < 1e-12:
        return 0.0
    analytic = hilbert(x)
    phase = np.unwrap(np.angle(analytic))
    return float(abs(phase[-1] - phase[0]) / (2 * np.pi))


# ---------------------------------------------------------------------
# 3. Siatka (identyczna z poprzednimi mostami)
# ---------------------------------------------------------------------
