"""Frozen first B4-Kitchen execution for CMU Kitchen S13 Brownie.

Reads the two downloaded source archives in place and emits one JSON report.
It intentionally has no tuning switches: all analysis choices are frozen in
docs/geometry/B4_KITCHEN_ADDENDUM_v0.1.md.
"""
...
RESULT_PATH = REPO / "docs" / "geometry" / "B4_KITCHEN_RESULT_v0.1.json"
WINDOW_SIZE = 50
EPS = 1e-9
...
def _permutation_test(x: np.ndarray, y: np.ndarray, seed: int) -> dict[str, float | int]:
    # Ranks do not change under a permutation.  Precomputing them preserves
    # exactly the same Spearman null while avoiding 10,000 redundant sorts.
    rx, ry = _rank(np.asarray(x, dtype=float)), _rank(np.asarray(y, dtype=float))
    rx -= rx.mean()
    ry -= ry.mean()
    denom = float(np.linalg.norm(rx) * np.linalg.norm(ry))
    if denom == 0.0:
        raise ValueError("constant series has undefined Spearman rho")
    observed = float(np.dot(rx, ry) / denom)
    rng = np.random.default_rng(seed)
    exceedances = 0
    for _ in range(N_PERMUTATIONS):
        rho = float(np.dot(rx, ry[rng.permutation(ry.size)]) / denom)
...
def _controls(n: int) -> dict[str, object]:
    ...
    positive_test = _permutation_test(positive, positive, SEED)
    negative_test = _permutation_test(_ar1(n, 0.8, 20_260_917), _ar1(n, 0.8, 20_260_918), SEED)
    return {
...
def main() -> None:
    ...
    main_test = _permutation_test(lambda_g, lambda_meta, SEED)
    if not controls["passed"]:
        verdict = "INCONCLUSIVE"
    elif main_test["pvalue"] < ALPHA:
        verdict = "SUPPORTED"
    else:
