# core/real_chrono_cone_bridge_v2.py
"""
real_chrono_cone_bridge_v2.py -- test glowny (PREREG SS7-8,
docs/geometry/PREREG_CHRONO_CONE_MS_BRIDGE_v0.2.md) metryki
`chrono_pendulum_ratio` (core/chrono_cone_bridge_v2.py) na TRZECH
realnych domenach uzywanych przez v0.1 (core/real_chrono_cone_bridge.py)
i wszystkie poprzednie mosty M/S<->topologia/G tego repo: lozyska CWRU,
sejsmika Ridgecrest 2019, BTC/USD -- TE SAME pliki, TE SAME rozmiary
okien co v0.1, dla bezposredniej porownywalnosci testu STABILNOSCI
ZNAKU (PREREG SS8, glowne pytanie tej sesji).

Uruchamiane WYLACZNIE po tym, jak core/chrono_cone_bridge_v2.py::
run_synthetic_controls() przeszlo (PASSED=True dla obu window_size) --
PRZESZLO (patrz konsola sesji), wiec ten plik moze byc uruchomiony.

Nic w tym pliku nie zostalo zmienione PO zobaczeniu wynikow -- wszystkie
stale sa przeniesione 1:1 z PREREG SS7, zamrozonej PRZED uruchomieniem
tego pliku, oraz 1:1 z core/real_chrono_cone_bridge.py (v0.1) gdzie
dotyczy danych/rozmiarow okien (jedyna zmiana: chrono_cone_ratio ->
chrono_pendulum_ratio, plus dodatkowa kolumna chrono_pendulum_net_turn).
"""
...
SEED = 0
ALPHA = 0.05
MIN_VALID_PER_GROUP = 10  # PREREG SS7: identyczne z v0.1
...
def _valid_pair(values: np.ndarray, aux: np.ndarray):
    mask = ~np.isnan(values)
    return values[mask], aux[mask]
...
def run_group_test(
    ...
    pos_segs = [pos_gen(0, int(s)) for s in seeds]
    neg_segs = [neg_gen(0, int(s)) for s in seeds]

    pos_ratio = np.array([chrono_pendulum_ratio(s) for s in pos_segs])
    neg_ratio = np.array([chrono_pendulum_ratio(s) for s in neg_segs])
    pos_turn = np.array([chrono_pendulum_net_turn(s) for s in pos_segs])
    neg_turn = np.array([chrono_pendulum_net_turn(s) for s in neg_segs])

    pos_ratio_valid, _ = _valid_pair(pos_ratio, pos_turn)
    neg_ratio_valid, _ = _valid_pair(neg_ratio, neg_turn)
    pos_turn_valid = pos_turn[~np.isnan(pos_turn)]
    neg_turn_valid = neg_turn[~np.isnan(neg_turn)]

    row = {
        "sigma": sigma,
        "n_valid_pos": len(pos_ratio_valid),
        "n_valid_neg": len(neg_ratio_valid),
        "n_total": n_windows,
    }
    if len(pos_ratio_valid) < MIN_VALID_PER_GROUP or len(neg_ratio_valid) < MIN_VALID_PER_GROUP:
        row.update({"status": "INCONCLUSIVE", "p": None, "r": None,
                     "median_pos": None, "median_neg": None,
                     "turn_status": "INCONCLUSIVE", "turn_p": None, "turn_r": None})
        return row

    result = mann_whitney_test(pos_ratio_valid, neg_ratio_valid)
    ...
    # sekundarna diagnostyka: chrono_pendulum_net_turn (PREREG SS5, NIE
    # uzywana do klasyfikacji SUPPORTED, raportowana obok)
    if len(pos_turn_valid) >= MIN_VALID_PER_GROUP and len(neg_turn_valid) >= MIN_VALID_PER_GROUP:
        turn_result = mann_whitney_test(pos_turn_valid, neg_turn_valid)
        turn_supported = turn_result.pvalue < ALPHA and abs(turn_result.effect_size_r) >= 0.3
        row.update({
            "turn_status": "SUPPORTED" if turn_supported else "NOT_SUPPORTED",
            "turn_p": turn_result.pvalue,
            "turn_r": turn_result.effect_size_r,
        })
    else:
        row.update({"turn_status": "INCONCLUSIVE", "turn_p": None, "turn_r": None})
    return row


# =======================================================================
# 1. Lozyska CWRU (identyczne pliki/okna co v0.1)
# =======================================================================
...
# =======================================================================
# 2. Sejsmika Ridgecrest 2019 (identyczne pliki/okna co v0.1)
# =======================================================================
...
# =======================================================================
# 3. BTC/USD -- BLOCK_SIZE=48h (identyczne z v0.1)
# =======================================================================
