# Historia README — GIA-TIMDR / TRM (Geometric Information Accelerator - Triangle Information Momentum Dynamics Resonance/Triangle Resonance Model)
...
Zasada „bez tuningu po wyniku” nie zabrania kalibracji przed testem:
rozwój na rozłącznych `train/calibration`, jawne zamrożenie, następnie
jednorazowy `holdout`. Zmiany techniczne w kalibracji wymagają nowej
wersji planu, a wynik po podejrzeniu holdoutu nie może być ponownie
ogłoszony potwierdzeniem na tych samych danych. Pełna reguła:
[`TIMDR_CALIBRATION_FREEZE_RULE.md`](docs/theory/TIMDR_CALIBRATION_FREEZE_RULE.md).
...
>
> **Dodatkowo (2026-09-22/23, zwykła kopia plików, NIE `git subtree` —
...
> trzech modułów z siostrzanego repo aplikacyjnego `MAGE-IN-IMAGE-DECODER`
> (ogólny detektor obrazu/wideo, NIE formalny fundament TIMDR jak cztery
> gałęzie wyżej) — `meta_dynamics_v1.py` (Λ-τ-ρ na polu ruchu wideo),
> `contour_curvature.py` (krzywizna konturów 2D, G-branch, hipoteza
> detekcji splicingu) i `vector_reference.py` (wykrywanie ruchu: optyczny
> przepływ + kalibracja na zdrowym nagraniu referencyjnym, metoda
> analogiczna do `TIMDR-Industrial-Predict`, nie nowa gałąź aksjomatyczna);
> żaden nie zależy od reszty repo źródłowego. Testy, skrypty real-data,
> dane i wyniki PREREG/RESULT (ρ NOT SUPPORTED, Λ częściowy trop;
> CONTOUR_CURVATURE częściowo supported przy N=4; wektor ruchu +MOG2 na
> CDnet 2014: makro F1 +0.03) zostają wyłącznie w repo źródłowym.
